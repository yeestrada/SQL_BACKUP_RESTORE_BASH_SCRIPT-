...
(for %%a in (%list%) do ( 
	...
	SQLCMD -S %SERVERNAMETO% -U %TOUSER% -P %TOKEY% -Q "RESTORE DATABASE %%a FROM DISK='%DEFAULTLOCALPATH%%COMPANY%_BACKUP_%DATESTAMP%\%%a.bak'" -o %DEFAULTLOCALPATH%%COMPANY%_BACKUP_%DATESTAMP%\log.txt
))
